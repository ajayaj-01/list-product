...
async function loadProducts() {
    try {
        ...
        // Extract categories
        allCategories = [...new Set(allProducts.map(product => product.category))];
...
function renderProducts() {
    ...
    loadMoreButton.style.display = displayedProducts.length < allProducts.length ? 'block' : 'none';
}
...
function filterProductsByCategory() {
    ...
    if (selectedCategories.length === 0) {
        ...
        displayedProducts = allProducts.filter(product => selectedCategories.includes(product.category));
    }
    ...
    try {
        ...
        productDetailContainer.innerHTML = `
            <div class="product-detail-content">
                ...
                <div class="info">
                    <h2>${product.title}</h2>
                    <p class="short-description">${product.description.slice(0, 100)}<span class="read-more">Read more</span></p>
                    <p class="full-description" style="display:none;">${product.description}</p>
                    <p class="price">$${product.price}</p>
                    <div class="stars">${createRatingStars(product.rating.rate)}(${product.rating.rate})</div>
                    <hr>
                    <h4>Quantity</h4>
                    <div class="quantity">
...
document.getElementById('load-more').addEventListener('click', () => {
    currentPage++;
    const nextProducts = allProducts.slice(currentPage * productsPerPage - productsPerPage, currentPage * productsPerPage);
